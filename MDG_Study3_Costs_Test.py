...
clicktime4global=''
Totalclick=''
diseases=[('Gwaronia'), ('Zymosis'), ('Descolada'),('Metalytis')]
tests=[('CAT'), ('XRAY'), ('MRI'), ('LAB')]
tests_copy=[('CAT'), ('XRAY'), ('MRI'), ('LAB')]
locationsdummy=[1,3,2,4]
costsdummy=[1,2,3]
...
genoutcome_map = {
'X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG//Images/fever.png': 1,
'X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG//Images/rash.png': 2,
'X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG//Images/migraine.png': 3,
'X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG//Images/ache.png': 4,
}
...
trials = data.TrialHandler(nReps=1, method=u'random', 
    ...
    trialList=data.importConditions('X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG/conditions1.xlsx'),
...
for thisTrial in trials:
    ...
    trialComponents.append(clicked_diagnoses)
    ...
    totalclick = 0
    testsRemaining = 4 
    diagnosis_clicks = {1: 0, 2: 0, 3: 0, 4: 0} 
    diagnosis_click_order = []  
    total_diagnosis_clicks = 0 
    ...
    while continueRoutine:
        ...
        # Check if diagnosis slider has been used
        if diagnosis.status == STARTED:
            selected_diagnosis = diagnosis.getRating()  # Get current selection

            # If a valid diagnosis is selected and hasn't been clicked before
            if selected_diagnosis in diagnosis_clicks and diagnosis_clicks[selected_diagnosis] == 0:
                diagnosis_clicks[selected_diagnosis] = 1  # Mark as clicked
                total_diagnosis_clicks += 1  # Increment total clicks

                # Store the click order
                diagnosis_click_order.append(selected_diagnosis)

                #print(f"Diagnosis {selected_diagnosis} clicked in order {total_diagnosis_clicks}")
    ...
    trials.addData('Trial', trials.thisTrialN+1)
    ...
    trials.addData('DiagnosisNumber', diagnosis.getRating())
    ...
    trials.addData('DiagnosisClickOrder', diagnosis_click_order)
    clicked_diagnoses = [diseases[idx - 1] for idx in diagnosis_click_order]
    trials.addData('ClickedDiagnoses', clicked_diagnoses)
...
while continueRoutine:
    ...
    if t >= 0.0 and backgroundinst1.status == STARTED:
        AIinstructionstext.text = 'IN THIS NEXT ROUND YOU WILL RECEIVE ADVICE FROM AN AI ADVISOR.\n\n THERE WILL ALSO BE SOME TRIALS WITHOUT ANY AI ADVICE. \n\nPLEASE READ THE ADVICE CAREFULLY BEFORE PROCEEDING. IT IS IMPORTANT TO PAY ATTENTION TO THE DETAILS PROVIDED BY EACH ADVISOR. \n\n IF YOU HAVE ANY QUESTIONS PLEASE ASK THE EXPERIMENTER. '
...
trials = data.TrialHandler(nReps=1, method=u'random', 
    extraInfo=expInfo, originPath=None,
    trialList=data.importConditions('X:/Psychology/ResearchProjects/JAHoffmann/PhDCremenAITeams/Projects/Study3/Medical_Diagnosis_Game/MDG/conditions2.xlsx'),
    seed=None, name='trials')
thisExp.addLoop(trials)  # add the loop to the experiment
blockClock.reset()
phase=2
...
for thisTrial in trials:
    ...
    # Calculate the most common and least common click orders
    if click_order_dict[genoutcome]:
        most_common_click_order_for_genoutcome = Counter(click_order_dict[genoutcome]).most_common(1)[0][0]
        least_common_click_order_for_genoutcome = Counter(click_order_dict[genoutcome]).most_common()[-1][0]
    ...
    DA_advice = f"THIS IS YOUR AI ADVISOR.\nIs there stronger evidence for any of the other diseases when the patient has had {symptom_name} and those test results?"
    FAC_advice = f"THIS IS YOUR AI ADVISOR.\nDo we have a majority opinion in the team for which disease it is, given the {symptom_name}?"
    ...
    # Initialize FAC advice variables at the start of the routine (outside the loop)
    FAC_advice_texts = [
        f"THIS IS YOUR AI ADVISOR.\nDo we have a majority opinion in the team for which disease it is, given the {symptom_name}?",
        "THIS IS YOUR AI ADVISOR.\nDo we now have a majority opinion on the diagnosis with the information from this first test result?",
        "THIS IS YOUR AI ADVISOR.\nDo we now have a majority opinion on the diagnosis with the information from this second test result?",
        "THIS IS YOUR AI ADVISOR.\nDo we now have a majority opinion on the diagnosis with the information from this third test result?",
        "THIS IS YOUR AI ADVISOR.\nDo we now have a majority opinion on the diagnosis with this final test result?"
    ]
    ...
    selected_index = rng.choice(len(advice_options))

    # FAC-specific variables
    FAC_index = 0  # Tracks which FAC advice text is being displayed
    clicked_shapes = set()  # Keeps track of shapes clicked during FAC
    adviceTriggered = False
    adviceActive = False

    # Track test shapes in a list 
    test_shapes = [Test1Shape, Test2Shape, Test3Shape, Test4Shape]

    while continueRoutine:

        random_advice = advice_options[selected_index]
        advice_name = advice_names[selected_index]
        ...
        for i, (testShape, randomShape, testImage, testText, clickVar, costText, testOutcome, clickOrderIdx) in enumerate(
            ...
            if testShape.status == STARTED and testsRemaining > 0:
                ...
                if mouse.isPressedIn(testShape) and eval(f'click{clickOrderIdx}') < 1:
                    ...
                    # Handle FAC-specific click logic
                    if adviceActive:
                        for i, shape in enumerate([Test1Shape, Test2Shape, Test3Shape, Test4Shape]):
                            if mouse.isPressedIn(shape) and i not in clicked_shapes:
                                
                                # Register the click
                                clicked_shapes.add(i)
                                FAC_index += 1

                                if FAC_index < len(FAC_advice_texts):
                                    PromptTextAdvice.setAutoDraw(False)
                                    PromptTextAdvice.text = FAC_advice_texts[FAC_index]
                                    PromptTextAdvice.setAutoDraw(True)

                                # Reset mouse click to avoid double clicks
                                mouse.clickReset()
                                break 
        ...
        if not adviceTriggered:
            if advice_name == 'DA':
                if diagnosis.getRating() is not None and clickDiagnosis < 1:  
                    adviceTriggered = True
                    adviceVisible = True
                    clickDiagnosis += 1

            # Check for FAC advice triggering
            if advice_name == "FAC" and not adviceTriggered:
                adviceTriggered = True
                adviceActive = True
                PromptTextAdvice.text = FAC_advice_texts[FAC_index]  # Show the first advice text
                PromptTextAdvice.setAutoDraw(True)
        ...
        # Toggle advice visibility for general clicks (excluding FAC-specific interactions)
        if adviceTriggered and advice_name != 'FAC' and adviceVisible and mouse.getPressed()[0]:
            adviceVisible = False  # Hide advice on next general click
            PromptTextAdvice.setAutoDraw(False)

        # Display or hide advice based on visibility state (excluding FAC-specific interactions)
        if advice_name != 'FAC':
            if adviceVisible:
        ...
        # Check for response and handle submit button click
